...
$('settings').onclick=()=>settingsDialog();$('about').onclick=()=>{showDialog('<div class="eyebrow">TOKYO PLAYTEST / 04 · 能力更新</div><h2>让全城加入反抗。</h2><p>摧毁出生点这一岸的两座黄色发光中枢，再过河推翻对岸 Boss，便完成东京人觉醒。<br>开局选一种变异。九种能力各三级：空气传播、枪？枪！、可感染发射器、狂杀、持续感染、急速、皮糙肉厚、生存与进化、听我号令。<br>步行已放慢；Shift 奔跑消耗体力。轻击 0.7 秒、重击 1.2 秒范围伤。<br>转化后的友军按敌方满状态砍半。点出「枪？枪！」后按 3–7 切换手枪、霰弹、步枪、狙击与 RPG。点出「可感染发射器」后按 2 切换。</p><button class="action" id="closeAbout">知道了</button>');$('closeAbout').onclick=hideDialog;};
...
function updateMissions(){while(missionReady(state)&&state.mission<4){const done=MISSIONS[state.mission][0];state.mission++;state.hp=Math.min(state.maxHp,state.hp+25);toast('任务完成 · '+done+' | 生命 +25',4);music.effect('upgrade');$('missionComplete').textContent='✓ '+done+' · 完成';missionTimer=3;saveRun();}
 ...
 if(bossReady(state)&&!boss.active){state.mission=Math.max(state.mission,4);saveRun();spawnQueue.jobs.length=0;spawnQueue.wait=0;alerted=false;boss.active=true;boss.root.visible=true;boss.x=world.bossSpawn.x;boss.z=world.bossSpawn.z;boss.attack=4;toast('两个秩序中枢已摧毁 · 前往对岸讨伐巨像',4);}
}
