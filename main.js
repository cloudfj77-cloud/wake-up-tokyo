...
$('settings').onclick=()=>settingsDialog();$('about').onclick=()=>{showDialog('<div class="eyebrow">TOKYO PLAYTEST / 04 · 能力更新</div><h2>让全城加入反抗。</h2><p>感染 40 人、摧毁三座中枢、推翻巨大黄色 Boss，完成东京人觉醒。<br>开局选一种变异。九种能力各三级：空气传播、枪？枪！、可感染发射器、狂杀、持续感染、急速、皮糙肉厚、生存与进化、听我号令。<br>步行已放慢；Shift 奔跑消耗体力。轻击 0.75 秒、重击 1.25 秒范围伤。<br>点出「枪？枪！」后按 3–7 切换手枪、霰弹、步枪、狙击与 RPG。开局 2 键为可感染发射器。</p><button class="action" id="closeAbout">知道了</button>');$('closeAbout').onclick=hideDialog;};
...
for(let i=tracers.length-1;i>=0;i--){const b=tracers[i];b.life-=dt;if(b.m){
 if(b.isPower){
  ...
  if(world.solid(b.x,b.z,.1)){if(b.friendly)world.breakAt(b.x,b.z,b.splash||.75,18,burst);b.life=0;}
  else if(b.friendly){
...
}if(b.life<=0){scene.remove(b.m||b.line);if(b.line){b.line.geometry.dispose();b.line.material.dispose();}if(b.isPower){b.m.geometry?.dispose?.();b.m.material?.dispose?.();}tracers.splice(i,1);}}}
