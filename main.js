...
function drawMap(){
 ...
 // 三个中枢始终保留编号；未摧毁时用醒目的黄色脉冲，摧毁后变暗并打叉。
...
function updateMissions(){while(missionReady(state)&&state.mission<4){const done=MISSIONS[state.mission][0];state.mission++;state.hp=Math.min(state.maxHp,state.hp+25);toast('任务完成 · '+done+' | 生命 +25',4);music.effect('upgrade');$('missionComplete').textContent='✓ '+done+' · 完成';missionTimer=3;saveRun();}
 // 演示版不再等待感染人数或前置任务：三个中枢一旦全部摧毁，就立即进入 Boss 战。
 if(bossReady(state)&&!boss.active){state.mission=Math.max(state.mission,4);saveRun();spawnQueue.jobs.length=0;spawnQueue.wait=0;alerted=false;boss.active=true;boss.root.visible=true;boss.x=world.bossSpawn.x;boss.z=world.bossSpawn.z;boss.attack=4;toast('三个秩序中枢已摧毁 · 巨像降临！',5);}
}
