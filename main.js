...
import {loadCharacterAsset,createCharacterVisual,setCharacterKind,playCharacterAttack,updateCharacterVisual,characterHeight,PROFESSIONS} from './characters.js';
import {block} from './world.js';
import {loadRiverside} from './riverside-world.js';
import {Soundtrack} from './music.js';
import {ABILITIES,ENEMIES,GUNS,POPULATION,STAMINA_MAX,makeState,makeUnit,hit,upgrade,choices,stepSimulation,damageAlly,distance,threat,outcome,speed,teamCount,hurtMother,missionReady,MISSIONS,spawnPlan,TUNE,resetTune,applyTune,setTuneValue,meleeSpec,gunInfection,guardWanted,levelCap,isRangedEnemy,hostileWindup,hostileSwingConnects} from './rules.js';
const $=id=>document.getElementById(id),canvas=$('game');
const music=new Soundtrack();let mode='loading',state=makeState(),units=[],player={x:0,z:-7,y:0,vy:0,crouch:false,roll:0,rollX:0,rollZ:1},world,asset;
...
function selectChoice(index){const a=choiceSet[index];if(!a||mode!=='upgrade')return;if(!upgrade(state,a.id))return;music.effect('upgrade');hideDialog();mode='playing';music.resume();if(a.id==='command')ensureGuards(true);if(a.id==='guns'&&state.abilities.guns===1)toast('解锁手枪 / 霰弹枪 · 按 2 / 3 切换',2.4);else toast(`获得 ${a.name} Lv.${state.abilities[a.id]}`,2);saveRun();}
...
function saveRun(){if(import.meta.env.DEV&&new URLSearchParams(location.search).has('test'))return;if(!asset||['loading','menu','ended','won'].includes(mode)||boss.active)return;try{localStorage.setItem(SAVE_KEY,JSON.stringify({version:6,state,units,player,yaw,pitch,cameraDistance,attackCd,breakCd,rushCd,reinforceTimer,reinforceDirection,totalReinforcements,alerted,objects:world.destructibles.map(o=>({hp:o.hp,dead:o.dead,removed:o.removed?[...o.removed]:[]}))}));}catch{}}
function loadSaved(){try{const s=JSON.parse(localStorage.getItem(SAVE_KEY));return s?.version===6&&s.state?.hp>0&&s.units?.length?s:null;}catch{return null;}}
...
function beginHostileAttack(u,spec,victim){
 const ranged=isRangedEnemy(spec);
 u.aiming=true;
 u.aim=hostileWindup(spec);
 const visual=visuals.get(u.id)?.v;
 if(!visual)return;
 visual.holdingGun=ranged;
 visual.holder.rotation.y=Math.atan2(victim.x-u.x,victim.z-u.z);
 if(!ranged)playCharacterAttack(visual,u.type===1||u.type===3?'break':'infect',Math.max(.75,(u.aim||.4)+.22));
}
function tickHostileAttack(u,spec,victim,nearest){
 const visual=visuals.get(u.id)?.v;
 if(visual){
  visual.holdingGun=isRangedEnemy(spec);
  visual.holder.rotation.y=Math.atan2(victim.x-u.x,victim.z-u.z);
 }
 if(u.aim>0)return;
 if(hostileSwingConnects(spec,nearest,world.clear(u,victim)))enemyFire(u,spec,victim);
 else{
  u.aiming=false;u.aim=0;u.attackCd=spec.interval;
  if(visual)visual.holdingGun=false;
 }
}
...
else{if((!alerted&&state.time<8)||d>(player.crouch?15:27+state.alert*3)){u.aiming=false;u.aim=0;const visual=visuals.get(u.id)?.v;if(visual){visual.holdingGun=false;updateCharacterVisual(visual,false,dt,d);}continue;}let victim=player,nearest=d;for(const a of allies){const dd=distance(u,a);if(dd<nearest){nearest=dd;victim=a;}}const spec=ENEMIES[u.type];tx=victim.x;tz=victim.z;pace=u.aiming||nearest<=spec.range*.82?0:spec.speed;if(u.aiming){pace=0;u.aim=Math.max(0,(u.aim||0)-dt);tickHostileAttack(u,spec,victim,nearest);}else if(nearest<spec.range&&u.attackCd<=0&&world.clear(u,victim)){beginHostileAttack(u,spec,victim);pace=0;}}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.04,(now-last)/1000);last=now;frameDelta=dt;if(mode==='playing'){
 ...
 const body=characterHeight('player')/asset.height*(state.abilities.frenzy>=3?1.35:1);playerVisual.model.scale.set(body,body*(player.crouch?.7:1),body);playerVisual.holdingGun=state.weapon>=0;updateCharacterVisual(playerVisual,moving,dt,0,sprinting);
...
window.gameSnapshot=()=>({mode,...state,team:teamCount(units),position:{...player},units:units.map(u=>({id:u.id,kind:u.kind,type:u.type,x:u.x,z:u.z,hp:u.hp,infection:u.infection,corpseTime:u.corpseTime})),characterModel:!!asset,sceneName:'日式河畔城市·体素版',buildings:world.buildings.length,spawn:world.spawn,renderCalls:renderer.info.render.calls,camera:{x:camera.position.x,y:camera.position.y,z:camera.position.z}});
