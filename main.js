...
import {ABILITIES,ENEMIES,POPULATION,LEVEL_CAP,makeState,makeUnit,hit,upgrade,choices,rerollChoice,hasRerollPool,hasLauncher,launcherSpec,consumeLauncherCharge,stepSimulation,damageAlly,distance,threat,outcome,speed,cooldownScale,teamCount,hurtMother} from './rules.js';
...
function updateChargeRing(){
 ...
 const level=state.abilities.launcher||0,charge=state.charge??0,ready=level>=3&&charge>=6,playing=mode==='playing'||mode==='paused';
 // 前两级不展示蓄力环，避免玩家误以为普通发射器也能释放强化炮弹。
 chargeRing.visible=level>=3&&playing;if(!chargeRing.visible)return;
...
function showConversionFeedback(events){
 // 中央反馈只奖励玩家亲手完成的感染，空气传播等间接感染继续放到侧边栏。
...
function applyHit(u,inf,damage,source='indirect'){const result=hit(state,u,inf,damage),landed=result!=='none',activeAttack=['melee','syringe','power'].includes(source);if(activeAttack&&landed)tutorialAction('hit');if(result==='converted'){if(activeAttack)tutorialAction('infect');pendingConversions.push({name:unitName(u),central:activeAttack});burst(u.x,STANDING_CHEST,u.z,0xc781ff,18);wave(u.x,u.z,.9,0xc781ff);}else if(result==='hit'&&source==='syringe')showInfectionHit(u);else if(result==='hit'&&activeAttack)burst(u.x,STANDING_CHEST,u.z,0xb86fff,6);else if(result==='hit')burst(u.x,STANDING_CHEST,u.z,0xebc5a6,4);else if(result==='killed'&&u.type>0)spawnPickup(u.x,u.z);return result;}
...
function primary(){if(mode!=='playing'||attackCd>0||switchTime>0||reloadTime>0||player.roll>0)return;raiseAlarm();playerVisual.holder.rotation.y=yaw;const f=forward();tutorialAction('attack');
 if(state.weapon<0){attackCd=.58*cooldownScale(state);playCharacterAttack(playerVisual,'infect');const u=acquire(2.8);if(u)applyHit(u,20,18,'melee');else world.breakAt(player.x+f.x*1.6,player.z+f.z*1.6,1.8,50,burst,world.heightAt(player.x,player.z)+STANDING_CHEST);if(boss.active&&!boss.dead&&distance(player,boss)<5)hitBoss(boss,75);music.effect('attack');return;}
 if(!launcherSpec(state)){state.weapon=-1;toast('尚未解锁可感染发射器',1.4);return;}if(state.clip<=0){reloadGun();return;}const strong=consumeLauncherCharge(state),spec=launcherSpec(state,strong);state.clip--;attackCd=spec.cooldown*cooldownScale(state);playCharacterAttack(playerVisual,'shoot');weaponPivot.position.y-=.05;const u=acquire(spec.range),end=u?{x:u.x,z:u.z}:{x:player.x+f.x*spec.range,z:player.z+f.z*spec.range};const d=Math.hypot(end.x-player.x,end.z-player.z)||1,shotHeight=player.crouch?CROUCH_CHEST:STANDING_CHEST,shotY=world.heightAt(player.x,player.z)+player.y+shotHeight,m=strong?createPowerOrb(player.x,shotY,player.z):block(scene,.1,.1,.7,0xc078ff,player.x,shotY,player.z);tracers.push({m,x:player.x,z:player.z,y:m.position.y,vy:strong?4.8:0,vx:(end.x-player.x)/d*spec.speed,vz:(end.z-player.z)/d*spec.speed,life:strong?4:.9,friendly:true,strong,isPower:strong,trail:0,damage:spec.damage,infection:spec.infection,pierce:spec.pierce,hitIds:new Set()});burst(player.x+f.x,player.y+shotHeight,player.z+f.z,strong?0xe4adff:0xba63ff,strong?22:10);music.effect(strong?'powerThrow':'shoot');if(!strong&&state.abilities.launcher>=3){music.effect('chargeTick',state.charge);if(state.charge===6){toast('六次发射完成 · 下一发触发强化炮弹',2);music.effect('chargeReady');}}
}
function reloadGun(){if(reloadTime||switchTime||state.weapon<0||state.clip>=state.clipMax)return;if(state.ammo[0]<=0){toast('针剂耗尽 · 徒手击败秩序单位或完成任务补充');return;}reloadDuration=1.5;reloadTime=reloadDuration;}
...
function weaponSelect(index){if(switchTime||index===state.weapon)return;if(index===0&&!hasLauncher(state)){toast('尚未解锁可感染发射器',1.4);return;}nextWeapon=index;switchTime=.4;attackCd=Math.max(attackCd,.4);toast(index<0?'切换 · 徒手':'切换 · 可感染发射器',.7);}
...
function choiceSlotHtml(a,index){const used=choiceRerolls[index],canRefresh=!used&&hasRerollPool(state,choiceSet,choiceSeen),label=used?'已刷新':canRefresh?'刷新这张':'没有其他能力';return `<div class="choiceSlot"><button class="card" data-choice="${index}"><kbd>${index+1}</kbd><span class="type">${a.group} / ${state.abilities[a.id]?'强化能力':'新能力'}</span><span class="icon">${a.icon}</span><strong>${a.name}</strong><em>Lv.${state.abilities[a.id]} → Lv.${state.abilities[a.id]+1}</em><small>${a.descriptions[state.abilities[a.id]]}</small></button><button class="reroll" data-reroll="${index}" ${canRefresh?'':'disabled'}>${label}</button></div>`;}
function bindChoiceActions(){document.querySelectorAll('[data-choice]').forEach(button=>button.onclick=()=>selectChoice(+button.dataset.choice));document.querySelectorAll('[data-reroll]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();refreshChoice(+button.dataset.reroll);});}
function refreshChoice(index){if(mode!=='upgrade'||choiceRerolls[index])return;const next=rerollChoice(state,choiceSet,index,Math.random,choiceSeen);if(!next){toast('本轮没有其他可选能力',1.2);return;}choiceSet=next;choiceRerolls[index]=true;choiceSeen.add(next[index].id);$('upgradeCards').innerHTML=choiceSet.map(choiceSlotHtml).join('');bindChoiceActions();music.effect('chargeTick',index+1);toast(`第 ${index+1} 张已刷新 · ${next[index].name}`,1.4);}
function selectChoice(index){const a=choiceSet[index];if(!a||mode!=='upgrade')return;if(!upgrade(state,a.id))return;if(playerVisual)makeAura(playerVisual.holder,AURA_COLORS.level,{coreSize:2.3,moteSize:.52,radius:.46,life:1});music.effect('upgrade');hideDialog();mode='playing';music.resume();if(a.id==='launcher'&&state.abilities.launcher===1)toast('解锁可感染发射器 · 按 2 切换',2.4);else if(a.id==='launcher'&&state.abilities.launcher===2)toast('发射器强化 · 感染针可以穿透',2.4);else if(a.id==='launcher'&&state.abilities.launcher===3)toast('发射 6 次后，下一发变为强化炮弹',2.4);else toast(`获得 ${a.name} Lv.${state.abilities[a.id]}`,2);saveRun();}
function chooseUpgrade(){mode='upgrade';music.pause();choiceSet=choices(state);if(!choiceSet.length){state.pending=0;mode='playing';music.resume();return;}choiceRerolls=choiceSet.map(()=>false);choiceSeen=new Set(choiceSet.map(a=>a.id));showDialog(`<div class="eyebrow">GENETIC RECOMBINATION / LV.${state.level}</div><h2>${state.history.length?'反抗，正在进化。':'选择你的第一种变异。'}</h2><p>战场已暂停。五种能力各三级；每张卡本次可以单独刷新一次，已经出现过的能力不会回来。<br>按 1 / 2 / 3 或点击卡片直接选择。</p><div class="cards" id="upgradeCards">${choiceSet.map(choiceSlotHtml).join('')}</div>`);bindChoiceActions();}
...
function togglePause(){if(mode==='playing'){mode='paused';music.pause();saveRun();showDialog(`<div class="eyebrow">SIMULATION PAUSED</div><h2>东京，暂时静止。</h2><p>街区进度自动保存；Boss 战从进场前存档重试。所有战斗计时已暂停。</p><div class="controlTable">WASD 移动 · 鼠标或右键拖动转向<br>左键 / J 普通攻击 · 空格跳跃 · Shift 翻滚 · C 蹲伏<br>1 徒手 · 2 发射器（解锁后）· E 装填 · R 召集 / 解散<br>感染优先结算：感染值满的这一击不会造成伤害。<br>空气传播 Lv2 可转化尸体；净化士兵在 6 米内每秒清除 8 感染。</div><button class="action" id="resume">继续觉醒 ↗</button><button class="action secondary" id="pauseSettings">设置</button><button class="action secondary" id="endRun">结束并复盘</button>`);$('resume').onclick=togglePause;$('pauseSettings').onclick=()=>settingsDialog('paused');$('endRun').onclick=()=>finish('ended');}else if(mode==='paused'){hideDialog();mode='playing';music.resume();}}
...
$('settings').onclick=()=>settingsDialog();$('about').onclick=()=>{showDialog('<div class="eyebrow">TOKYO PLAYTEST / 03</div><h2>让全城加入反抗。</h2><p>感染 40 人、摧毁三座中枢、推翻巨大黄色 Boss，完成东京人觉醒。<br>开局只有徒手攻击；在变异卡中选到「可感染发射器」后按 2 使用。<br>发射器二级可以穿透，三级连续发射 6 次后获得强化炮弹。每张变异卡本次可以刷新一次。<br>摧毁橙色秩序中枢可以压制后续增援；击败秩序单位可以补充针剂。<br>利用掩体、蹲伏、跳跃和翻滚生存。</p><button class="action" id="closeAbout">知道了</button>');$('closeAbout').onclick=hideDialog;};
...
for(let i=tracers.length-1;i>=0;i--){const b=tracers[i];b.life-=dt;if(b.m){
 ...
 else{b.x+=b.vx*dt;b.z+=b.vz*dt;b.m.position.set(b.x,b.y??1.2,b.z);if(world.solid(b.x,b.z,.1)){if(b.friendly)world.breakAt(b.x,b.z,.75,18,burst,b.y);b.life=0;}else if(b.friendly){if(boss.active&&!boss.dead&&distance(b,boss)<3.4){hitBoss(boss,24+(state.abilities.launcher||0)*8);burst(b.x,3,b.z,0xc978ff,12);b.life=0;}else for(const u of units)if(!u.dead&&!u.converted&&u.kind!=='corpse'&&!b.hitIds?.has(u.id)&&distance(b,u)<.72){applyHit(u,b.infection??12,b.damage??4,'syringe');burst(b.x,STANDING_CHEST,b.z,0xb76bff,12);if((b.pierce||0)>0){b.hitIds?.add(u.id);b.pierce--;}else b.life=0;break;}}else if(distance(b,player)<.58&&(b.y??1.2)>world.heightAt(player.x,player.z)+player.y+.12&&(b.y??1.2)<world.heightAt(player.x,player.z)+player.y+(player.crouch||player.roll>0? .85:2)&&invincible<=0){hurtMother(state,50);invincible=.35;b.life=0;shake=.2;music.effect('hurt');}else for(const a of units)if(a.kind==='ally'&&!a.dead&&distance(b,a)<.6){damageAlly(a,b.damage);b.life=0;break;}}
...
function drawMap(){
 ...
 for(const b of world.buildings){ctx.fillStyle=b.dead?'#566273':'#869397';ctx.fillRect(px(b.x-b.w/2),pz(b.z+b.d/2),b.w/132*200,b.d/132*160);}
 for(const u of units){if(u.dead||u.kind==='corpse')continue;ctx.fillStyle=u.converted?'#b976ed44':u.type?'#e27c68':'#e6e7d5';if(u.converted){ctx.fillRect(px(u.x)-5,pz(u.z)-5,10,10);ctx.fillStyle='#c190ef';}ctx.fillRect(px(u.x)-1,pz(u.z)-1,2.5,2.5);}
 // 中枢在小地图上始终使用高亮圆标和“中”字，不能再混成普通的四像素小方块。
 for(const tower of world.towers)if(!tower.dead){const x=px(tower.x),z=pz(tower.z),pulse=5+Math.sin(performance.now()*.008)*1.2;ctx.save();ctx.shadowColor='#ff762e';ctx.shadowBlur=10;ctx.fillStyle='#ffad5c';ctx.beginPath();ctx.arc(x,z,pulse,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle='#402313';ctx.font='bold 8px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('中',x,z+.5);ctx.restore();}
...
function hud(){
 ...
 const launcherUnlocked=hasLauncher(s);$('primaryLabel').textContent=switchTime>0?'切换中…':s.weapon<0?'徒手':'可感染发射器';$('breakCd').textContent=player.y>.1?'空中':player.crouch?'蹲伏':'站立';$('rushCd').textContent=rushCd>0?rushCd.toFixed(1)+'s':'就绪';$('chargeText').hidden=(s.abilities.launcher||0)<3;$('ammo').innerHTML=`<span class="${s.weapon<0?'selected':''}">1 徒手</span>${launcherUnlocked?`<span class="${s.weapon===0?'selected':''}">2 发射器</span><span>紫液针剂 <b>${s.clip} / ${s.ammo[0]}</b></span>`:'<span>升级时可解锁发射器</span>'}<span>${reloadTime>0?'装填 '+reloadTime.toFixed(1)+'s':'E 装填 · R 召集'}</span>`;$('crosshair').classList.toggle('locked',!!target);$('targetHint').textContent=target?`${target.type===0?PROFESSIONS[target.profession]:ENEMIES[target.type].name} · ${Math.ceil(distance(player,target))}m`:'';$('damageVignette').style.opacity=shake>0?.7:0;drawMap();drawMissionMapMarker();drawLabels();
}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.04,(now-last)/1000);last=now;frameDelta=dt;if(mode==='playing'){
 ...
 playerVisual.model.scale.y=(characterHeight('player')/assets.player.height)*(player.crouch?.7:1);playerVisual.holdingGun=state.weapon===0&&hasLauncher(state);updateCharacterVisual(playerVisual,moving,dt,0,false);updateChargeRing();
 syringe.g.visible=state.weapon===0&&hasLauncher(state)&&player.roll<=0;weaponPivot.position.set(player.x-Math.cos(yaw)*.5,world.heightAt(player.x,player.z)+player.y+(player.crouch?CROUCH_CHEST:STANDING_CHEST),player.z+Math.sin(yaw)*.5);weaponPivot.rotation.set(switchTime>0?Math.sin(switchTime/.4*Math.PI)*1.1:reloadTime>0?.5:0,yaw+.38,0);syringe.liquid.scale.y=.3*(.4+.6*state.clip/state.clipMax);
 const activeLauncher=launcherSpec(state);target=acquire(state.weapon<0?3.2:activeLauncher?.range??3.2);if(mouseHeld||keys.KeyJ)primary();const before=state.infected;stepSimulation(state,units,player,dt,state.tutorialDone?mode:'tutorial');if(state.infected>before)state.maxChain=Math.max(state.maxChain,state.infected-before);runAI(dt);updateVisuals(dt);updatePickups(dt);updateEffects(dt);
