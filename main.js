...
const music=new Soundtrack();let mode='loading',state=makeState(),units=[],player={x:0,z:-7,y:0,vy:0,crouch:false,roll:0,rollX:0,rollZ:1},world,assets;let hurtTimer=0;
...
function primary(){if(mode!=='playing'||attackCd>0||switchTime>0||reloadTime>0||player.roll>0)return;raiseAlarm();playerVisual.holder.rotation.y=yaw;const f=forward();
 if(state.weapon<0){attackCd=.58*cooldownScale(state);playCharacterAttack(playerVisual,'infect');const u=acquire(2.8);if(u)applyHit(u,20,18,'melee');else world.breakAt(player.x+f.x*1.6,player.z+f.z*1.6,1.8,50,burst);if(boss.active&&!boss.dead&&distance(player,boss)<5)hitBoss(boss,75);music.effect('attack');}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.04,(now-last)/1000);last=now;frameDelta=dt;if(mode==='playing'){
 ...
 playerVisual.model.scale.y=(characterHeight('player')/assets.player.height)*(player.crouch?.7:1);playerVisual.holdingGun=state.weapon===0;updateCharacterVisual(playerVisual,moving,dt,0,false);
 ...
 if(state.infected>conversionCount){conversionTimer=1.1;$('infectionFeedback').innerHTML=`<span>☣</span><b>觉醒成功 ×${state.infected-conversionCount}</b><small>加入反抗群落</small>`;conversionCount=state.infected;music.effect('convert');}const result=outcome(state);if(result)finish(result);else if(state.pending&&state.time-state.lastPick>=50)chooseUpgrade();saveTimer+=dt;if(saveTimer>5){saveRun();saveTimer=0;}if(lastHp!==null&&state.hp<lastHp)hurtTimer=.6;lastHp=state.hp;
